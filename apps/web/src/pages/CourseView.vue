...
<template>
  <div class="mecourse-app">
    ...
    <section class="mecourse-workspace">
      <header class="mecourse-header">
        <label class="mecourse-search">
          <span aria-hidden="true">⌕</span>
          <input
            v-model="query"
            type="search"
            placeholder="Курсы, уроки, темы"
            @focus="activeSection = 'catalog'"
          />
          <button v-if="query" type="button" aria-label="Очистить поиск" @click="query = ''">×</button>
        </label>

        <div class="mecourse-header__actions">
          <button class="mcrn-header-icon mecourse-icon-button" type="button" aria-label="Сменить тему" @click="app.toggleTheme()">
          ...
          <button class="mcrn-header-icon mcrn-header-icon--notify mecourse-icon-button" type="button" aria-label="Уведомления">
            <svg viewBox="0 0 24 24" aria-hidden="true">
          ...
          <button class="mcrn-header-profile mecourse-profile" type="button" aria-label="Открыть профиль">
            <span>
              <svg viewBox="0 0 24 24" aria-hidden="true">
